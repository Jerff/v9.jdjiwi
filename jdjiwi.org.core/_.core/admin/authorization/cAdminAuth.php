...
cLoader::library('admin/authorization/cAdminAuthSession');

abstract class cAdminAuth {
    ...
    public function __construct() {
        $this->setSession(new cAdminAuthSession($this, $this->getName()));
    ...
    protected function setError($login) {
        ...
        if ($row) {
            ...
            if ($error > 10) {
                ...
                if ($error < 15)
                    $time = time() + 60 * ($error - 10);
                else if ($error < 20)
                    $time = time() + 2 * 60 * ($error - 10);
                else if ($error < 30)
                    $time = time() + 3 * 60 * ($error - 10);
                else
                    $time = time() + 4 * 60 * ($error - 10);
                if ($error >= 20) {
