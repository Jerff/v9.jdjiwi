...
if($sectionId) {
    ...
    cmfSeo::set('title', $section['title']);
    cmfSeo::set('keywords', $section['keywords']);
    cmfSeo::set('description', $section['description']);
...
if($brandId) {
    $brand = $sql->placeholder("SELECT id, name, uri, title, keywords, description FROM ?t WHERE id=? AND visible='yes'", db_brand, $brandId)
                   ->fetchAssoc();
    cmfSeo::set('brand', $brand['name']);
    if(!$sectionId) {
        cmfSeo::set('title', $brand['title']);
        cmfSeo::set('keywords', $brand['keywords']);
        cmfSeo::set('description', $brand['description']);
    }
